...
public abstract class CorpusJob implements NLPJob {
	...
	public void run(JobManager mgr) throws Exception {
		RepositoryReader reader = getReader(mgr);
		PatchCorpusWriter writer = getWriter(mgr);
	...
	protected PatchCorpusWriter getWriter(JobManager mgr) throws Exception {
	...
	protected abstract RepositoryReader getReader(JobManager mgr) throws Exception;
